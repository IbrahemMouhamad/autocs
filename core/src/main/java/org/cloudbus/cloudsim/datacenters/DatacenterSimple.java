...
public class DatacenterSimple extends CloudSimEntity implements Datacenter {
    ...
    /** @see #getStateHistory() */
    private final List<DatacenterStateHistoryEntry> stateHistory;
     ...
     *
     * <p>
     * <b>NOTE:</b> To change such attributes, just call the respective setters.
     * </p>
     *
     ...
     *                           simulation the Entity belongs
     * @param hostList           list of {@link Host}s that will compound the
     *                           Datacenter
     ...
     * {@link #getDatacenterStorage() storage}.
     *
     * @param simulation         the CloudSim instance that represents the
     *                           simulation the Entity belongs
     * @param hostList           list of {@link Host}s that will compound the
     *                           Datacenter
     * @param vmAllocationPolicy the policy to be used to allocate VMs into hosts
     * @param storageList        the storage list to attach to the
     *                           {@link #getDatacenterStorage() datacenter storage}
     */
    public DatacenterSimple(
            final Simulation simulation,
            final List<? extends Host> hostList,
     ...
     *                           Datacenter
     * @param vmAllocationPolicy the policy to be used to allocate VMs into hosts
     * @param storage            the {@link #getDatacenterStorage() storage} for
     *                           this Datacenter
    ...
    private boolean processCloudletEvents(final SimEvent evt) {
        return switch (evt.getTag()) {
            ...
            // Pauses a previously submitted Cloudlet, but the sender asks for an
            // acknowledgement
            ...
            // Resumes a previously submitted Cloudlet, but the sender asks for an
            // acknowledgement
     ...
     * @param ack      indicates if the Broker is waiting for an ACK after the
     *                 Datacenter
     ...
     * This is the minimum value between the {@link #getSchedulingInterval()} and
     * the given time
     ...
     * has to be performed.
     *
     * @param nextFinishingCloudletTime the predicted completion time of the
     *                                  earliest finishing cloudlet
     *                                  (which is a relative delay from the current
     *                                  simulation time),
     *                                  or {@link Double#MAX_VALUE} if there is no
     *                                  next Cloudlet to execute
     * @return next time cloudlets processing will be updated (a relative delay from
    ...
    protected double getCloudletProcessingUpdateInterval(final double nextFinishingCloudletTime) {
        ...
        /*
         * If a scheduling interval is set, ensures the next time that Cloudlets'
         * processing
    ...
    protected boolean finishVmMigration(final SimEvent evt, final boolean ack) {
        ...
        // Updates processing of all Hosts to get their latest state before migrating
        // VMs
        ...
        if (suitability.fully()) {
            ...
            /*
             * When the VM is destroyed from the source host, it's removed from the
             * vmExecList.
        ...
        if (event == null || event.getTime() > clock()) {
            // Updates processing of all Hosts again to get their latest state after the VMs
            // migrations
            updateHostsProcessing();
        }

        if (suitability.fully())
            LOGGER.info("{}: Migration of {} from {} to {} is completed.", getSimulation().clockStr(), vm, sourceHost,
                    targetHost);
        else
            LOGGER.error(
        ...
        onVmMigrationFinishListeners
                .forEach(listener -> listener.update(DatacenterVmMigrationEventInfo.of(listener, vm, suitability)));
        return true;
    }
    ...
    /**
     * Sends an ACK to the DatacenterBroker that submitted the Cloudlet for
     * execution
     ...
     *
     * @param cloudlet the cloudlet to respond to DatacenterBroker if it was created
     *                 or not
     * @param ack      indicates if the Broker is waiting for an ACK after the
     *                 Datacenter
     *                 receives the cloudlet submission
     ...
     *
     * <p>
     * <b>This is an expensive operation for large scale simulations.</b>
     * </p>
     */
    ...
    /**
     * Indicates if it's time to check if suitable Hosts are available to migrate
     * VMs
    ...
    public void requestVmMigration(final Vm sourceVm, Host targetHost) {
        ...
        if (Host.NULL.equals(targetHost)) {
            LOGGER.warn("{}: {}: No suitable host found for {} in {}", sourceVm.getSimulation().clockStr(),
                    getClass().getSimpleName(), sourceVm, this);
            return;
        }

        final Host sourceHost = sourceVm.getHost();
        final double delay = timeToMigrateVm(sourceVm, targetHost);
        final String msg1 = Host.NULL.equals(sourceHost) ? String.format("%s to %s", sourceVm, targetHost)
    ...
    private double timeToMigrateVm(final Vm vm, final Host targetHost) {
        return vm.getRam().getCapacity()
                / bitesToBytes(targetHost.getBw().getCapacity() * getBandwidthPercentForMigration());
    }
    ...
    public final Datacenter setVmAllocationPolicy(final VmAllocationPolicy vmAllocationPolicy) {
        ...
        if (vmAllocationPolicy.getDatacenter() != null && vmAllocationPolicy.getDatacenter() != Datacenter.NULL
                && !this.equals(vmAllocationPolicy.getDatacenter())) {
    ...
    public Host getHostById(final long id) {
        return hostList.stream().filter(host -> host.getId() == id).findFirst().map(host -> (Host) host)
                .orElse(Host.NULL);
    }
    ...
    public <T extends Host> Datacenter addHost(final T host) {
        if (vmAllocationPolicy == null || vmAllocationPolicy == VmAllocationPolicy.NULL) {
            throw new IllegalStateException(
                    "A VmAllocationPolicy must be set before adding a new Host to the Datacenter.");
        }
    ...
    public final void setPowerModel(final PowerModelDatacenter powerModel) {
        ...
        if (powerModel.getDatacenter() != null && powerModel.getDatacenter() != Datacenter.NULL
                && !this.equals(powerModel.getDatacenter())) {
            throw new IllegalStateException(
                    "The given PowerModel is already assigned to another Datacenter. Each Datacenter must have its own PowerModel instance.");
        }
    ...
    public Datacenter setHostSearchRetryDelay(final double delay) {
        if (delay == 0) {
            throw new IllegalArgumentException(
                    "hostSearchRetryDelay cannot be 0. Set a positive value to define an actual delay or a negative value to indicate a new Host search must be tried as soon as possible.");
        }
    ...
    /**
     * Enables storing Datacenter state history.
     *
     * @see #getStateHistory()
     */
    public void enableStateHistory() {
        // enable state history for all hosts
        this.getHostList().forEach(host -> host.enableStateHistory());
        this.stateHistoryEnabled = true;
    }

    /**
     * Disable storing Datacenter state history.
     *
     * @see #getStateHistory()
     */
    public void disableStateHistory() {
        // disable state history for all hosts
        this.getHostList().forEach(host -> host.disableStateHistory());
        this.stateHistoryEnabled = false;
    }

    /**
     * Checks if Datacenter state history is being collected and stored.
     *
     * @return
     */
    ...
    protected void addStateHistory(final double currentTime) {
        if (!stateHistoryEnabled) {
            return;
        }

        // we suppose that, state history is enabled for all hosts
        addStateHistoryEntry(
                currentTime,
                this.getHostList().stream()
                        .map(host -> host.getStateHistory().get(host.getStateHistory().size() - 1).allocatedMips())
                        .reduce(0.0, Double::sum),
                this.getHostList().stream()
                        .map(host -> host.getStateHistory().get(host.getStateHistory().size() - 1).requestedMips())
                        .reduce(0.0, Double::sum),
                this.getHostList().stream()
                        .map(host -> host.getRamUtilization())
                        .reduce(0L, Long::sum),
                this.getHostList().stream()
                        .map(host -> host.getBwUtilization())
                        .reduce(0L, Long::sum),
                this.getHostList().stream()
                        .map(host -> host.getStateHistory().get(host.getStateHistory().size() - 1).active() ? 1.0 : 0.0)
                        .reduce(0.0, Double::sum));
    }

    /**
     * Adds a datacenter state history entry.
     *
     * @param time          the time
     * @param allocatedMips the allocated mips
     * @param requestedMips the requested mips
     * @param activeHosts   the number of active hosts
     */
    private void addStateHistoryEntry(
            final double time,
            final double allocatedMips,
            final double requestedMips,
            final long ramUtilization,
            final long bwUtilization,
            final double activeHosts) {
        final var newState = new DatacenterStateHistoryEntry(time, allocatedMips, requestedMips, ramUtilization,
                bwUtilization, activeHosts);
    ...
    /**
     * Gets a <b>read-only</b> datacenter state history.
     * This List is just populated if {@link #isStateHistoryEnabled()}
     *
     * @return the state history
     * @see #enableStateHistory()
     */
    public List<DatacenterStateHistoryEntry> getStateHistory() {

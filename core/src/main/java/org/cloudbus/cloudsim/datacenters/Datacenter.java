...
public interface Datacenter extends SimEntity, PowerAware<PowerModelDatacenter>, TimeZoned, Sizeable {
    ...
    /**
     * Sends an event to request the migration of a {@link Vm} to a given target
     * {@link Host}
     * on this Datacenter.
    ...
    /**
     * Sends an event to request the migration of a {@link Vm} to some suitable
     * {@link Host}
     * on this Datacenter. A suitable Host will try to be found when the migration
     * request message
    ...
    /**
     * Physically expands the Datacenter by adding a List of new Hosts (physical
     * machines) to it.
    ...
    /**
     * Physically expands the Datacenter by adding a new Host (physical machine) to
     * it.
     * Hosts can be added before or after the simulation has started.
     * If a Host is added during simulation execution,
     * in case VMs are added dynamically too, they
     ...
     *
     * <p>
     * If an ID is not assigned to the given Host,
     * the method assigns one.
     * </p>
     *
     ...
     * processing of such cloudlets, it only defines in which interval the
     * processing
     ...
     *
     * <p>
     * The default value is zero, which indicates no scheduling interval
     ...
     * if you want to collect some simulation data in a defined time interval.
     * </p>
     *
     ...
     */
    List<DatacenterStateHistoryEntry> getStateHistory();
}

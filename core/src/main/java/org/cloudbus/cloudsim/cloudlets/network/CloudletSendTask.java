...
public class CloudletSendTask extends CloudletTask {
     ...
     * @throws RuntimeException         when a NetworkCloudlet was not assigned to
     *                                  the Task
     * @throws IllegalArgumentException when the source or destination Cloudlet
     *                                  doesn't have an assigned VM
     */
    public VmPacket addPacket(final NetworkCloudlet destinationCloudlet, final long bytes) {
        ...
        final var packet = new VmPacket(
                getCloudlet().getVm(), destinationCloudlet.getVm(), -1,
     ...
     *
     * @param destinationCloudlet destination cloudlet to send packets to
     * @param flowId              flow id which this packet is belong to, used in
     *                            SDN forwarding table
     * @param bytes               the number of data bytes of the packet to create
     * @return the created packet
     * @throws RuntimeException         when a NetworkCloudlet was not assigned to
     *                                  the Task
     ...
     *                                  doesn't have an assigned VM
     */
    public VmPacket addPacket(final NetworkCloudlet destinationCloudlet, final long flowId, final long bytes) {
        if (getCloudlet() == null) {
            throw new IllegalStateException("You must assign a NetworkCloudlet to this Task before adding packets.");
        }
        if (!getCloudlet().isBoundToVm()) {
        ...
        final var packet = new VmPacket(
                getCloudlet().getVm(), destinationCloudlet.getVm(), flowId,
                bytes, getCloudlet(), destinationCloudlet);
        packetsToSend.add(packet);
        return packet;
    }
